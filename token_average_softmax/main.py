...
class EERunner(object):
    def __init__(self, conf, device=None):
        ...
        #print(self.arg.hps)的实体

        self.train = os.path.join(self.arg.data_path, "test.json")
        self.test = os.path.join(self.arg.data_path, "test.json")
        self.dev = os.path.join(self.arg.data_path, "dev.json")
    ...
    def load_model(self, fine_tune):
        """ :param fine_tune model name that previously trained"""
        config, un_used = BertConfig.from_pretrained(self.arg.bert_model, num_labels=self.arg.hps["trigger_BIOTag_num"], output_hidden_states=True, return_unused_kwargs=True)
        print("unused args = {}".format(un_used))
        mymodel = Joint3EEModel(self.arg.hps, device=self.device, bert_model=self.arg.bert_model,
    ...
    def run(self, Train=True, Test=False):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(self.arg.bert_model, do_lower_case=bool(self.arg.do_lower_case))

        consts.TOKEN_MASK_TYPE = self.arg.token_mask_type
        self.train_set = self.construct_dataset(self.train, keep_events=1, skip_sample=self.arg.skip_sample, tokenizer=self.tokenizer)  # load datafiles and transinto field
        self.dev_set = self.construct_dataset(self.dev, tokenizer=self.tokenizer)
        self.test_set = self.construct_dataset(self.test, tokenizer=self.tokenizer)
        #self.dev_set = self.construct_dataset(self.dev, keep_events=1, skip_sample=self.arg.skip_sample, tokenizer=self.tokenizer)
        #self.test_set = self.construct_dataset(self.test, keep_events=1, skip_sample=self.arg.skip_sample, tokenizer=self.tokenizer)
        ...
        if Train:
            ...
            os.system("cp network/*.py {}".format(self.arg.output_path + "/code"))
    ...
    def define_input_field(self):
        ...
        self.WordsTokensValidField = MultiTokenTensorField(use_vocab=False, batch_first=True, dtype=torch.float32,
                                                           pad_token=0., fix_length=(None, consts.CUTOFF))
    ...
    def buil_field_vocab(self):
        ...
        self.WordsTokensField.build_vocab(self.tokenizer, self.train_set.TOKENS)

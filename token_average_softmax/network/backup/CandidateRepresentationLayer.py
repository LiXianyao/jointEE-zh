...
class CandidateRepresentationLayer(nn.Module):
    ...
    def forward(self, word_repr, candidates_idx, anchor_loc, anchor_cls, batch_candidate_num=None,
                            max_candidate_num=None, nonzero=True, candidate_label=None):
        '''
        ...
        # get candidates : BATCH_SIZE, batch_candidate_num, anchor_len, REPR_DIM
        batch_candidates_anchor_repr, batch_candidates_label, batch_candidate_num, candidate_len, candidate_mask, \
        ...
        anchor_repr_size = batch_candidates_anchor_repr.size()[:]
        batch_candidates_anchor_repr = batch_candidates_anchor_repr.view\
            (anchor_repr_size[0] * anchor_repr_size[1], anchor_repr_size[2], anchor_repr_size[3])
        batch_candidates_repr = self.inception_repr.forward(batch_candidates_anchor_repr,
                                                                   pooling_height=anchor_repr_size[2])  # [b*n, 1, r]
        # [BATCH_SIZE, batch_candidate_num, REPR_DIM]
        batch_candidates_repr = batch_candidates_repr.view(anchor_repr_size[0], anchor_repr_size[1], anchor_repr_size[3])
    ...
    def get_candidates_repr(self, word_repr, candidates_idx, anchor_loc, anchor_cls, anchor_num, batch_candidate_num=None,
                            max_candidate_num=None, nonzero=True, candidate_label=None):
        BATCH_SIZE, SEQ_LEN, anchor_num, anchor_len, REPR_DIM = word_repr.size()[:]
        ...
        batch_candidates_repr = word_repr[batch_idx, wid, aid, :, :].view(BATCH_SIZE, batch_candidate_num, anchor_len, REPR_DIM)
        ...
        if not self.training and candidate_label is not None:
            invalid_loc = torch.nonzero(candidate_label == 0)
            candidate_mask[invalid_loc[:, 0]] = 0.
        ...
        batch_candidates_repr = batch_candidates_repr * candidate_mask.unsqueeze(-1).unsqueeze(-1)
